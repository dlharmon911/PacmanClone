...
namespace pacman
{
	...
	namespace font
	{
		...
		{
			static const uint8_t default_font_data[256][GLYPH_SIZE] =
			{
			...
			if (data)
			{
				...
				if (font)
				{
					...
					if (font->m_glyphs)
					{
						...
						for (int32_t i = 0; i < count; ++i)
						{
							font->m_glyphs[i] = al_create_bitmap(GLYPH_SIZE, GLYPH_SIZE);

							if (font->m_glyphs[i])
							{
								glyph::modify(reinterpret_cast<glyph_t*>(font->m_glyphs[i]), data + i * GLYPH_SIZE);
							}
							else
							{
		...
		namespace glyph
		{
			...
			bool modify(glyph_t* glyph, const uint8_t* data)
			{
				...
				if (glyph && data)
				{
					...
					for (int32_t j = 0; j < GLYPH_SIZE; ++j)
					{
						uint8_t p = data[j];
						for (int32_t i = 0; i < GLYPH_SIZE; ++i)
						{
							float c = 0.0f;
							if (p & 0x1)
							{
							...
							al_put_pixel((GLYPH_SIZE - 1) - i, j, al_map_rgba_f(c, c, c, c));
							p = p >> 1;
						}
					}
			...
			{
				uint8_t data[GLYPH_SIZE] = { 0, 0, 0, 0, 0, 0, 0, 0 };
				for (int32_t i = 0; i < (GLYPH_SIZE << 1); i = i + 2)
				{
		...
		void draw(const font_t* font, color_t color, const point_t& point, alignment_t alignment, const std::string& text)
		{
			if (font)
			{
				...
				float w = (length * static_cast<float>(GLYPH_SIZE));
				...
				for (int32_t c : text)
				{
					...
					draw(font, color, p, c);
					x += static_cast<float>(GLYPH_SIZE);
				}
			}
		}
		...
		void get_data(bitmap_t* bitmap, uint8_t data[GLYPH_SIZE], int32_t x = 0, int32_t y = 0)
		{
		...
		font_t* load_from_json_file(const std::string& filename)
		{
			...
			uint8_t* data = new uint8_t[count * GLYPH_SIZE];
			if (!data)
			{
				return nullptr;
			}
		...
		font_t* load_from_bitmap_file(const std::string& filename, int32_t start)
		{
			...
			int32_t width = al_get_bitmap_width(bitmap) / GLYPH_SIZE;
			int32_t height = al_get_bitmap_height(bitmap) / GLYPH_SIZE;

			uint8_t* data = new uint8_t[width * height * GLYPH_SIZE];
			if (!data)
			{
			...
			for (int32_t j = 0; j < height; ++j)
			{
				for (int32_t i = 0; i < width; ++i)
				{
					...
					get_data(bitmap, data + index * GLYPH_SIZE, i * GLYPH_SIZE, j * GLYPH_SIZE);
				}
			}
